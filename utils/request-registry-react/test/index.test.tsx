...
describe("request-registry-react", () => {
  describe("useGetEndPoint", () => {
    it("renders loading state", () => {
      const userEndpoint = createGetEndpoint<{ id: string }, { name: string }>({
        url: ({ id }) => `/user/${id}`
      });
      ...
      const UserDetails = (props: { id: string }) => {
        ...
        if (endpointState.state !== "load") {
          return <div>loading</div>;
        }
        return <div>{endpointState.data.name}</div>;
      };
    ...
    it("rerenders data if cache is invalidated", async () => {
      ...
      const Runs = () => {
        ...
        return <div>{endpointState.data.run}</div>;
      };
    ...
    it("will take care of a slow outdated request", async () => {
      ...
      mockEndpointOnce(userEndpoint, async () => ({ name: "Fast", age: 1 }), 1);
