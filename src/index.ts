...
export interface EnpointGetOptions<TKeys, TResult, TKeysBind = TKeys> {
	 ...
	 * A custom loader
	 */
	loader?: (url: string, headers: { [key: string]: string }) => Promise<TResult>;
	/**
	 * Success handler
	 */
...
export function createGetEndpoint<TKeys, TResult>(
	...
	const api: EndpointGetFunction<TKeys, TResult> = Object.assign(
		function transmitFunction(keys: TKeys) {
			...
			// Execute request
			const ajaxResultPromise = api.loader(url.value, headers.value);
			// Store in cache
			if (cacheKey) {
 ...
 */
function getCacheKey(url: Cachable, header: Cachable<{ [key: string]: string }>): string | null {
